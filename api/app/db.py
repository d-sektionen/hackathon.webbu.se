...
async def add_user(name: str, password: str, db: Connection) -> User:
    user: User | None = await db.fetchrow("INSERT INTO users (name, password) VALUES ($1, $2) RETURNING *", name, password)
    if user is None:
...
async def add_session(user_id: UUID, db: Connection) -> Session:
    session: Session | None = await db.fetchrow("INSERT INTO sessions (user_id) VALUES ($1) RETURNING *", user_id)
    if session is None:
